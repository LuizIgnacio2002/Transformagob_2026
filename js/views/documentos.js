...
(function () {
  ...
  const { esc, badge, fmtDate, emptyState } = App.components;
  const { t } = App.i18n;

  const KIND_ICON = { referencia: "📄", identidad: "🪪", examen: "🔬", adjunto: "📎" };

  async function render(ctx) {
    const docs = await App.api.getDocuments(ctx.session.patientId);
    return `
    <div class="stack">
      <div class="page-head">
  ...
  function mount(root, ctx) {
    ...
    input.addEventListener("change", async () => {
      ...
      try {
        ...
        const doc = await App.api.uploadDocument(ctx.session.patientId, file);
