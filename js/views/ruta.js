...
(function () {
  ...
  const { esc, badge, fmtDate, STATE_META, ttsButton } = App.components;
  const { t } = App.i18n;

  async function render(ctx) {
    const api = App.api;
    const pid = ctx.session.patientId;
    const [patient, journey] = await Promise.all([api.getPatient(pid), api.getJourney(pid)]);
  ...
  function mapNode(m, i, journey) {
    ...
    const state = (stage && stage.state) || "pending";
    return `
