...
(function () {
  ...
  const { esc, badge, fmtDate, dayMonth, ttsButton, emptyState } = App.components;
  const { t } = App.i18n;
  ...
  async function render(ctx) {
    ...
    const [patient, journey, appts, notifs] = await Promise.all([
      api.getPatient(pid), api.getJourney(pid), api.getAppointments(pid), api.getNotifications(pid),
    ]);
    ...
    const initials = (patient.alias && patient.alias[0]) || patient.name[0];
