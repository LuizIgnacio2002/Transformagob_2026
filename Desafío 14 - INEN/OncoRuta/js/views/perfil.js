...
export async function render(ctx) {
  ...
  const patient = ctx.session.patientId ? await getPatient(ctx.session.patientId) : null;
...
export function mount(root) {
  root.querySelector("#open-a11y")?.addEventListener("click", () => {
    document.querySelector(".a11y-fab")?.click();
  });
}
