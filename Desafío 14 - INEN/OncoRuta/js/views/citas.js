...
import { getAppointments, confirmAppointment } from "../api.js";
import { esc, badge, fmtDate, dayMonth, emptyState } from "../components.js";
import { toast } from "../notifications.js";
import { t } from "../i18n.js";

export async function render(ctx) {
  const appts = await getAppointments(ctx.session.patientId);
...
export function mount(root) {
  ...
  root.querySelectorAll("[data-confirm]").forEach((b) =>
    b.addEventListener("click", async () => {
      ...
      try {
        await confirmAppointment(b.dataset.confirm);
        toast("¡Cita confirmada! Te enviaremos un recordatorio.", "ok");
        const item = b.closest(".cita-item");
        b.replaceWith(Object.assign(document.createElement("span"), { className: "badge badge--ok", textContent: "✓ Confirmada" }));
        item.querySelector(".cita-body .badge").outerHTML = badge("confirmada", "ok");
      } catch (err) {
