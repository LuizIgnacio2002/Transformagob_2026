...
export function changeFont(dir) {
  const p = getPrefs();
...
export function toggleContrast() {
  const p = getPrefs();
  const next = p.contrast === "high" ? "normal" : "high";
  setPref("contrast", next);
  applyPrefs();
}

export function toggleReadable() {
  const p = getPrefs();
...
export function speak(text, btn) {
  ...
  u.lang = getLang() === "qu" ? "es-PE" : "es-PE"; // voz en español (no hay voz quechua estándar)
  u.rate = 0.95;
  u._btn = btn;
...
/* ---------- Panel flotante de accesibilidad ---------- */
export function mountA11yPanel() {
  ...
  fab.addEventListener("click", () => {
    ...
    if (open) panel.querySelector("button, [tabindex]")?.focus();
  });

  // Delegación de eventos del panel
  panel.addEventListener("click", (e) => {
    const act = e.target.closest("[data-a11y]")?.dataset.a11y;
    if (!act) return;
...
export function initTTSDelegation() {
