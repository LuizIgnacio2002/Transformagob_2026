...
> 📂 La aplicación vive en la **raíz del repositorio** (`index.html`) para que GitHub Pages
> la detecte automáticamente. La documentación original del desafío está en
> [`Desafío 14 - INEN/`](./Desaf%C3%ADo%2014%20-%20INEN/).
...
SPA modular en **HTML + CSS + JavaScript nativo, sin frameworks ni dependencias**.
Cada archivo es un script clásico que se registra en el espacio de nombres global
`window.App` y se carga en orden desde `index.html`. Gracias a esto **funciona abriendo
`index.html` directamente** (doble clic / `file://`), sin necesidad de un servidor.

```
/ (raíz del repositorio)
├── index.html              # Shell + carga de módulos (entrada de la app)
├── COMO_DESPLEGAR.txt      # Guía de despliegue (GitHub Pages, etc.)
...
└── Desafío 14 - INEN/      # Documentación original del desafío (fuentes .txt)
```
...
**La forma más simple:** abre `index.html` con doble clic (o arrástralo a tu navegador).
No necesita servidor ni Python.

Opcionalmente, si prefieres servirlo por HTTP (por ejemplo para probar como en producción):
...
Luego abre **http://localhost:8080**. En VS Code también puedes usar **Live Server**.

Inicia sesión con una cuenta de demostración (ver tabla arriba).

### 🌐 Despliegue en línea (sin Python)

La app es 100% estática: funciona en **GitHub Pages**, Netlify, Vercel, etc.
Con los archivos en la raíz y Pages apuntando a `main` / `(root)`, la URL queda limpia:

```
https://luizignacio2002.github.io/Transformagob_2026/
```

Ver **[`COMO_DESPLEGAR.txt`](./COMO_DESPLEGAR.txt)** para el paso a paso completo.
